...

{
  ...
  function modeFromActiveOverlayTrigger() {
    const mapEl = document.getElementById(mapId);
    if (!mapEl) return null;

    const section = mapEl.closest(".cr-section") || document;
    const triggers = Array.from(
      section.querySelectorAll('.narrative-col .new-trigger[data-focus-on="cr-map"]')
    );
    ...
    const text = (best.textContent || "").toLowerCase();
    if (text.includes("healthy texas women")) return "htw";
    if (text.includes("family planning program")) return "fpp";
    return "population";
  }
  ...
  async function installModeWatcher() {
    for (let i = 0; i < 120; i++) {
      if (getLeafletMap() && getLayerControlRoot()) break;
      await sleep(50);
    }

    const installedKey = "__crMapModeWatcherInstalled";
    if (globalThis[installedKey]) return null;
    globalThis[installedKey] = true;

    const modeKey = "__crMapModeLastApplied";
    let ticking = false;

    const syncMode = async () => {
      ticking = false;
      const mode = modeFromActiveOverlayTrigger();
      if (!mode) return;
      if (globalThis[modeKey] === mode) return;
      globalThis[modeKey] = mode;
