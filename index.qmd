...
```{=html}
<span class="cr-map2-mobile-values">
South Texas Family Planning &amp; Health Corporation (STFPHC) mobile unit has driven -- miles so far, out of -- total planned miles.
</span>
<style>
  .cr-map2-mobile-values { display: none; }
  @media (max-width: 768px) {
    .cr-map2-mobile-values {
      display: block;
      margin-top: 0.5rem;
    }
  }
...
service_bbox <- st_bbox(highlight_map)
service_bounds_js <- sprintf(
  "[[%f, %f], [%f, %f]]",
  service_bbox["ymin"], service_bbox["xmin"],
  service_bbox["ymax"], service_bbox["xmax"]
)

# Build Leaflet widget
m <- leaflet(width = "100vw", height = "100svh", options = leafletOptions(zoomControl = FALSE, scrollWheelZoom = FALSE, minZoom = 5)) %>%
  addPolygons(
    data = texas_map,
...
"  if (isMobileViewport) {
",
"    // Lock map height to initial viewport pixels on phones to avoid URL-bar resize jitter.
",
...
",
"    if (!isMobileViewport) map.fitBounds(all.getBounds(), { padding: [20, 20] });
",
"  } catch(e) {}

",
"  var serviceBounds = L.latLngBounds(", service_bounds_js, ");
",
",
"  var servicePadding = isMobileViewport ? [24, 24] : [20, 20];
",
"  var isRefittingServiceArea = false;
",
"  var lastServiceRefitAt = 0;
",
"  var fullServiceZoom = map.getBoundsZoom(serviceBounds, false, servicePadding);
",
"  \n",
"  var baseIconW = ", marker_w, ";
",
"  var baseIconH = ", marker_h, ";
",
"  var mobileIconScale = isMobileViewport ? Math.max(0.60, Math.min(0.78, window.innerWidth / 460)) : 1;
",
"  var iconW = Math.round(baseIconW * mobileIconScale);
",
"  var iconH = Math.round(baseIconH * mobileIconScale);
",
"
",
"  function refitServiceArea() {
",
"    var now = Date.now();
",
"    if (now - lastServiceRefitAt < 250) return;
",
"    lastServiceRefitAt = now;
",
"    isRefittingServiceArea = true;
",
"    map.fitBounds(serviceBounds, { padding: servicePadding });
",
"    setTimeout(function(){ isRefittingServiceArea = false; }, 0);
",
"  }
",
"  function keepTracerCentered() {
",
"    if (!tracer) return;
",
"    map.panTo(tracer.getLatLng(), { animate: false });
",
"  }
",
"  function isServiceAreaInView() {
",
"    return map.getBounds().pad(0.02).contains(serviceBounds);
",
"  }
",
"  function enforceViewportRules() {
",
"    if (isRefittingServiceArea) return;
",
"    if (!tracer && !isServiceAreaInView()) {
",
"      refitServiceArea();
",
"    }
...
",
",
"    map.fitBounds(serviceBounds, { padding: servicePadding, animate: false });
",
"    map.on('zoomend moveend', keepTracerCentered);
",
"  } else {
",
"    map.on('moveend zoomend', enforceViewportRules);
",
"    map.on('moveend zoomend', keepTracerCentered);
",
"    map.on('resize', refitServiceArea);
",
"    refitServiceArea();
",
"  }
...
"  }
",
"  function buildMobileNarrative(distanceDriven) {
",
"    return 'STFPHC mobile unit has driven <strong>' + formatNumber(distanceDriven*0.62, 1) + ' miles</strong> so far, out of <strong>' + formatNumber(totalPlannedDistance*0.62, 1) + ' total planned miles</strong>.';
",
"  }
",
"  var mobileNarrativeOverlay = null;
",
"  function ensureMobileNarrativeOverlay() {
",
"    if (!isMobileViewport || mobileNarrativeOverlay) return;
",
"    mobileNarrativeOverlay = document.createElement('div');
",
"    mobileNarrativeOverlay.style.position = 'absolute';
",
"    mobileNarrativeOverlay.style.left = '12px';
",
"    mobileNarrativeOverlay.style.right = '12px';
",
"    mobileNarrativeOverlay.style.bottom = '12px';
",
"    mobileNarrativeOverlay.style.zIndex = '900';
",
"    mobileNarrativeOverlay.style.background = 'rgba(255, 255, 255, 0.96)';
",
"    mobileNarrativeOverlay.style.border = '1px solid rgba(83, 40, 79, 0.25)';
",
"    mobileNarrativeOverlay.style.borderRadius = '10px';
",
"    mobileNarrativeOverlay.style.boxShadow = '0 6px 18px rgba(0,0,0,0.14)';
",
"    mobileNarrativeOverlay.style.padding = '10px 12px';
",
"    mobileNarrativeOverlay.style.fontSize = '13px';
",
"    mobileNarrativeOverlay.style.lineHeight = '1.35';
",
"    mobileNarrativeOverlay.style.color = '#2f2a2f';
",
"    mobileNarrativeOverlay.style.pointerEvents = 'none';
",
"    el.style.position = 'relative';
",
"    el.appendChild(mobileNarrativeOverlay);
",
"  }
",
"  function setMobileNarrative(distanceDriven) {
",
"    if (!isMobileViewport) return;
",
"    var mobileNarrativeEl = document.querySelector('.cr-map2-mobile-values');
",
"    if (!mobileNarrativeEl) return;
",
"    mobileNarrativeEl.innerHTML = buildMobileNarrative(distanceDriven);
",
"  }
...
",
",
"      if (!isMobileViewport) tracer.bindPopup(buildTracerTooltip(feature, gradientState.baseDistanceDriven), {
",
...
",
",
"      if (!isMobileViewport) tracer.openPopup();
",
"      setMobileNarrative(gradientState.baseDistanceDriven);
",
"    } else {
",
...
",
",
"      if (!isMobileViewport) tracer.setPopupContent(buildTracerTooltip(feature, gradientState.baseDistanceDriven));
",
"      setMobileNarrative(gradientState.baseDistanceDriven);
",
"    }
...
",
"      map.panTo([b[1], b[0]], { animate: false });
",
...
",
",
"      if (!isMobileViewport) tracer.setPopupContent(buildTracerTooltip(feature, distanceDriven));
",
"      setMobileNarrative(distanceDriven);
",
"      idx++;
",
"    }, stepMs);
