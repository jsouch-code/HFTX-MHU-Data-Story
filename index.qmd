...

{
  ...
  function modeFromActiveOverlayTrigger() {
    const idx = (typeof crTriggerIndex === "number") ? crTriggerIndex : -1;
    const triggers = Array.from(document.querySelectorAll(".new-trigger"));
    const trigger = triggers[idx];
    if (!trigger) return null;

    const focusOn = (trigger.getAttribute("data-focus-on") || "").trim();
    if (focusOn !== "cr-map") return null;
  ...
  async function ensureReadyAndApply() {
    for (let i = 0; i < 120; i++) {
    ...
    const key = "__crMapModeLastApplied";
    if (globalThis[key] === mode) return;
    globalThis[key] = mode;

    await applyMode(mode);
  }

  await ensureReadyAndApply();
  null
}
