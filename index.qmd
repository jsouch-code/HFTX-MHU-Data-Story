...
| The impact on women’s health has been especially severe. Nearly half of Texas counties (46.5%) are classified as [maternity care deserts]{#cr-t1}, meaning they lack access to birthing facilities or maternity care providers altogether. [@fontenot2023maternity] For thousands of women, even basic pregnancy care requires traveling far from home.
...
**Healthy Texas Women (HTW)** is a Medicaid waiver, limited-benefit program that provides healthcare and family planning services to women aged 15-44 with low income and no insurance. [@hhsc_whp_2025]  
Services covered include:  
• Birth control  
• Cancer screenings  
• Chronic disease testing  & treatment  @cr-map
...
This map shows the route^[Distance estimates are approximate and were calculated using routes from ZIP code centroids rather than exact service locations; as a result, distances may not reflect true travel paths or starting points.] for the mobile clinic operated by the South Texas Family Planning & Health Corporation (STFPHC). Their mobile clinic traveled to 196 locations across 18 counties in 2025, reaching many communities with no other health care access.[@mobile_stops_2025]
...
",
"    iconAnchor: [", round(marker_w/2), ", ", round(marker_h/2), "]
",
"  });
",
"  var tracer = null;
...
",
"      tracer.bindTooltip(buildTracerTooltip(feature, gradientState.baseDistanceDriven), {
",
"        permanent: true,
",
"        direction: 'top',
",
"        offset: [0, -", round(marker_h/2), "],
",
"        opacity: 0.95,
",
"        className: 'route-live-tooltip'
",
"      }).openTooltip();
",
"    } else {
...
",
"      tracer.setTooltipContent(buildTracerTooltip(feature, gradientState.baseDistanceDriven));
",
"    }
",
"    tracerHeading = null;
...
"    var idx = 1;
",
"    var segDurationMs = Math.max(35, Math.floor(partMs / pointsPerPart));
",
"
",
"    function animateSegment() {
",
"      if (idx >= pts.length) { setTimeout(done, pausePartMs); return; }
",
...
",
"        var raw = Math.min(1, (ts - startTs) / segDurationMs);
",
"        var eased = raw * raw * (3 - 2 * raw);
",
"        var lng = a[0] + (b[0] - a[0]) * eased;
",
"        var lat = a[1] + (b[1] - a[1]) * eased;
",
"        tracer.setLatLng([lat, lng]);
",
...
",
"        tracer.setTooltipContent(buildTracerTooltip(feature, distanceDriven));
",
"        idx++;
",
"        animateSegment();
...
| In 2025, there were served [2,576 patients]{#cr-t8} [@mobile_clientsserved_2025] across [15,000 square miles]{#cr-t9} of Texas using its mobile unit.^[Area represents the combined land area of the 18 counties served by the mobile unit in 2025 and is intended to approximate the geographic service area.]
...
To learn more about this project, contact Mariana Sierra at <msierra@healthyfutures-tx.org>.
...
{{< modal toggle target="modal-scrollable" label="ⓘ" >}}

:::: {#modal-scrollable scrollable="true"}

####### How to Navigate the Data Story

This interactive story combines narrative, maps, and data visualizations to explore access to mobile women’s healthcare services across Texas.

- **Scroll through the story** to move between sections and activate different visualizations.
- **Hover over maps, charts, and graphics** to reveal additional details and contextual information.
- **Click or tap interactive elements** where available to explore data more closely.
- Some graphics may support **zooming, panning, or filtering** depending on the device and screen size.
- Narrative sections and visuals are connected — as you scroll, the graphics update to reflect the current discussion.

This project is designed to be exploratory. Readers are encouraged to move between sections, revisit visualizations, and engage with the data at their own pace.
